 ...
 */
#define RY(mW, mH)                                                             \
  ((mH) - ((mW) * (BHS + (1 - 2 * BWS) / 1.5 + (2 * SB + 1) / 2.0) +           \
           ROBOTSIZE / 2.0))
 ...
 */
#define WY(mW, mH)                                                             \
  ((mH) - ((mW) * (BHS + (1 - 2 * BWS) / 1.5 + 0.5 * SB) + SWS / 2.0))
 ...
 */
#define TSBY(mW, mH)                                                           \
  ((mH) - (mW) * (BHS + (1 - 2 * BWS) / 1.5 + (3 * SB + 1) / 2))
/* mw * bhs +                   // button width
 * mw * (1 - 2 * bws) / 1.5 +   // two button paddings (top and bot)
...
#define TOP_SAMPLE_BACKGROUND_RECT(mW, mH)                                     \
  QRectF(SBX(mW), TSBY(mW, mH), SBS(mW), SBS(mW))
#define BOT_SAMPLE_BACKGROUND_RECT(mW, mH)                                     \
  QRectF(SBX(mW), BSBY(mW, mH), SBS(mW), SBS(mW))
...
public:
   ...
   */
  SampleWall(QRectF rect, Menu *parent)
      : Wall(rect.x(), rect.y(), rect.width(), rect.height(), parent) {
    MyItem::setFlag(QGraphicsItem::ItemIsMovable, false);
  }
};
